...
export default function BlogListingPage() {
  ...
  // Background glow parallax (Framer is fine here — it's invisible if frozen)
  ...
  return (
    ...
    >
      {/* ============================================================ */}
      {/*  SUN ANIMATION — pure CSS keyframes (see <style> at bottom).  */}
      {/*  NOT Framer, so MotionConfig / reduced-motion can't freeze it.*/}
      {/* ============================================================ */}
      <style>{`
        @keyframes sunDrift {
      ...
      <MotionConfig reducedMotion="user">
            ...
            {featured && (
              ...
              >
                <Link href={`/blog/${featured.slug}`} className="group block">
                  <div className="grid grid-cols-1 overflow-hidden rounded-3xl border border-emerald-300/60 bg-white shadow-lg shadow-emerald-900/10 transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-emerald-900/20 lg:grid-cols-2">
                    <div className="relative aspect-[16/10] overflow-hidden lg:aspect-auto">
                      ...
                      <span className="absolute left-4 top-4 rounded-full bg-amber-500 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white shadow-sm">
                        Featured
                      </span>
                    </div>
                    ...
                    <div className="flex flex-col justify-center gap-4 p-6 sm:p-8 lg:p-10">
                      <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-emerald-700">
                        <span className="rounded-full bg-emerald-100 px-3 py-1">{featured.tag}</span>
                        <span>{featured.date}</span>
                        <span>·</span>
                        <span>{featured.readTime}</span>
                      </div>
                      <h2 className="font-display text-2xl font-black leading-tight text-black transition-colors group-hover:text-emerald-800 sm:text-3xl lg:text-4xl">
                        {featured.title}
                      </h2>
                      <p className="text-sm leading-relaxed text-neutral-700 sm:text-base">
                        {featured.excerpt}
                      </p>
                      <div className="flex items-center gap-2 pt-2 text-sm font-bold text-emerald-800">
                        <span>{featured.author}</span>
                      </div>
                      <span className="mt-2 inline-flex items-center gap-2 text-sm font-bold text-emerald-700 transition-colors group-hover:text-emerald-900">
                        Read Article
              ...
              {rest.map((post) => (
                <motion.article key={post.slug} variants={fadeUp}>
                  <Link href={`/blog/${post.slug}`} className="group block h-full">
                    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-emerald-300/60 bg-white shadow-sm shadow-emerald-900/5 transition-all duration-500 hover:-translate-y-2 hover:border-emerald-400 hover:shadow-xl hover:shadow-emerald-900/15">
                      ...
                      <div className="flex flex-1 flex-col gap-3 p-5 sm:p-6">
                        ...
                        <h3 className="font-display text-lg font-black leading-snug text-black transition-colors group-hover:text-emerald-800 sm:text-xl">
                          {post.title}
                        </h3>
                        ...
                        <span className="mt-3 h-1 w-8 rounded-full bg-emerald-500 transition-all duration-500 group-hover:w-full" />
                      </div>
                    </div>
                  </Link>
                </motion.article>
        ...
        {/* ===================== Trust Section ===================== */}
        <section className="relative overflow-hidden border-t border-emerald-300/40 bg-gradient-to-b from-[#cdebd2] to-[#bce2c2] py-20 sm:py-24 lg:py-32">
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-16">
              ...
              >
                <div>
                  <motion.span variants={fadeUp} className="mb-4 inline-flex items-center gap-2 text-[11px] font-extrabold uppercase tracking-[0.22em] text-emerald-800">
                    <span className="h-px w-6 bg-emerald-600" aria-hidden />
                    {trustData.eyebrow}
                  </motion.span>
                  <motion.h2 variants={fadeUp} className="font-display text-4xl font-black tracking-tight text-black sm:text-5xl">
                    {trustData.heading}
                  </motion.h2>
                  <motion.p variants={fadeUp} className="mt-4 text-base font-medium leading-relaxed text-emerald-950/80">
                    {trustData.intro}
                  </motion.p>
                </div>
                ...
                <motion.div variants={fadeUp} className="grid grid-cols-2 gap-4">
                  {trustData.stats.map((stat, i) => (
                    <div key={i} className="rounded-2xl border border-emerald-300/50 bg-white/50 p-5 shadow-sm backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:bg-white/80 hover:shadow-md">
                      <div className="font-display text-3xl font-black text-emerald-800">{stat.value}</div>
                      <div className="mt-1 text-[10px] font-bold uppercase tracking-wider text-emerald-900/70">
                        {stat.label}
                      </div>
                    </div>
                  ))}
              ...
              <motion.div
                ...
                className="rounded-3xl border border-emerald-300/60 bg-white p-4 shadow-xl shadow-emerald-900/10 sm:p-6 lg:col-span-7"
              >
...
/* ================================================================== */
function SunMark() {
  // unique gradient id per instance (two suns on the page)
  const gid = useId().replace(/:/g, "");

  return (
    <div className="relative h-full w-full">
      {/* Soft radial halo (CSS glow pulse) */}
      <div
