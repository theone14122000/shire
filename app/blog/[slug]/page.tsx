...
function SectionBlock({ section }: { section: BlogSection }) {
  switch (section.type) {
    case "paragraph":
      return (
        <p className="text-base leading-[1.9] text-neutral-700 sm:text-lg">
          {section.text?.split("\n").map((line, i, arr) => (
            <span key={i}>
              {line}
              {i < arr.length - 1 && <br />}
    ...
    case "heading":
      if (section.level === 3) {
        return (
          <h3 className="mt-10 font-display text-xl font-bold tracking-tight text-black sm:text-2xl">
            {section.text}
          </h3>
        );
      }
    ...
    case "image":
      return (
        <figure className="mt-8 overflow-hidden rounded-2xl border border-emerald-200/60 shadow-sm">
          ...
          {section.alt && (
            <figcaption className="bg-emerald-50/60 px-6 py-3.5 text-center text-xs font-medium text-emerald-700/60">
              {section.alt}
            </figcaption>
          )}
        </figure>
    ...
    case "youtube":
      return (
        <div className="mt-8 overflow-hidden rounded-2xl border border-emerald-200 bg-emerald-100/50 shadow-sm">
          <div className="relative aspect-video">
            <iframe
    ...
    <>
      <main className="min-h-screen bg-[#f4faf5] font-sans text-black">
        ...
        <article className="relative z-10 -mt-20 mx-auto max-w-4xl px-6 pb-20 sm:-mt-28 sm:px-8 lg:px-12 lg:pb-28">
          {/* Meta card */}
          <div className="rounded-[2rem] border border-emerald-200/60 bg-white p-8 shadow-sm sm:p-10 lg:p-12">
            <Link
              href="/blog"
            ...
            <div className="mt-14 border-t border-emerald-100 pt-10 text-center">
              ...
              <Link
                href="/#book"
                className="group mt-5 inline-flex items-center gap-2.5 rounded-full bg-emerald-700 px-8 py-4 text-sm font-bold text-white shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:bg-emerald-800 hover:shadow-xl"
              >
                Book Your Stay
                <svg
                  width="14"
